...
class MyoMain():
    ...
    def disconnect(self):
        ...
        self.mr.emg_list
        self.mr.label_list
        dataframe = pd.DataFrame(list(zip(self.mr.emg_list, self.mr.label_list, self.mr.trial_num_list, self.mr.mask_list)), columns = ['rawEMG', 'Label', 'Trial_num', 'mask'])
        
        if (os.path.exists(self.mr.ID)):
            session = int(natsort.natsorted(glob2.glob(f"{self.mr.ID}/**.csv"))[-1].split('S')[1].split('.')[0])
            dataframe.to_csv(f"{self.mr.ID}/{self.mr.ID}_{self.mr.Side}_emgData_S{session + 1}.csv")
        else:
            os.mkdir(self.mr.ID)
            dataframe.to_csv(f"{self.mr.ID}/{self.mr.ID}_{self.mr.Side}_emgData_S1.csv")
